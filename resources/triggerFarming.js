...
module.exports = async function call(commonData) {
    ...
    var ModelBasedFarming = await compile('ext/modelBasedFarmingManager/impl/ModelBasedFarmingManager')

    var osFarmingAddress = await organization.methods.get(commonData.grimoire.COMPONENT_KEY_OS_FARMING).call()
    var osFarming = new web3.eth.Contract(ModelBasedFarming.abi, osFarmingAddress);
    await blockchainCall(osFarming.methods.rebalanceRewardsPerBlock, commonData.fromAddress, {from : commonData.from})

    var dividendsFarmingAddress = await organization.methods.get(commonData.grimoire.COMPONENT_KEY_DIVIDENDS_FARMING).call()
    var dividendsFarming = new web3.eth.Contract(ModelBasedFarming.abi, dividendsFarmingAddress);

    /*if(web3.currentProvider.blockchainConnection) {
        await web3.eth.sendTransaction({
            from : commonData.fromAddress,
            to : dividendsFarmingAddress,
            value : utilities.numberToString(60*1e18)
        })
    }*/

    await blockchainCall(dividendsFarming.methods.rebalanceRewardsPerBlock, commonData.fromAddress, {from : commonData.from})
