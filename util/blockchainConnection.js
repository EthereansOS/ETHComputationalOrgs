    ...
    init: global.blockchainConnection = global.blockchainConnection || new Promise(async function(ok, ko) {
        try {
            ...
            if (process.env.blockchain_connection_string) {
                ...
                process.env.fork_block_number && (options.fork_block_number = process.env.fork_block_number);
                var block = await new Web3(process.env.blockchain_connection_string).eth.getBlock(process.env.fork_block_number || "latest");
            ...
            var unlock = async function unlock(error, response) {
                ...
                if(accountsToUnlock.length === 0) {
                    if (!response || !response.result) {
                        return ko(new Error((response && response.result) || response));
                    }
