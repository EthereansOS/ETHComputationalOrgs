...
module.exports = function transactionDebugger(web3, callback) {
    var debugBlock = function debugBlock(blockNumber, callback) {
        return web3.eth.getBlock(blockNumber).then(block => Promise.all(block && block.transactions.map(transactionHash => debugTransaction(transactionHash, web3)))).then(callback).catch(console.error);
    };
